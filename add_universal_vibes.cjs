...
const cities = db.prepare(`
  SELECT DISTINCT city FROM chat_rooms 
  WHERE city IS NOT NULL AND city != ''
`).all();

console.log(`Found ${cities.length} cities to update...`);

// First, delete old vibe rooms (keep main borough/city rooms)
db.prepare(`DELETE FROM chat_rooms WHERE room_type = 'vibe'`).run();
console.log('Cleared old vibe rooms');

// Add universal vibes for each city
let addedCount = 0;

cities.forEach(({ city }) => {
  console.log(`\nAdding vibes for ${city}...`);
  
  // Get the main room for this city (if it exists)
  const mainRoom = db.prepare(`
    SELECT id FROM chat_rooms 
    WHERE city = ? AND room_type IN ('city', 'borough')
    LIMIT 1
  `).get(city);
  
  const parentRoomId = mainRoom ? mainRoom.id : null;
  ...
  NIGHTLIFE_VIBES.forEach(vibe => {
    ...
    `).run(vibe.name, slug, city, vibe.emoji, vibe.description, parentRoomId, vibe.sort);
    addedCount++;
  });
  ...
  db.prepare(`
    INSERT INTO chat_rooms (name, slug, city, room_type, category, emoji, description, parent_room_id, sort_order)
    VALUES (?, ?, ?, 'vibe', 'flex', ?, ?, ?, ?)
  `).run(SPONTANEOUS.name, slug, city, SPONTANEOUS.emoji, SPONTANEOUS.description, parentRoomId, SPONTANEOUS.sort);
  addedCount++;
});

console.log(`\n✅ Added ${addedCount} universal vibe rooms`);
