...
print(f"=== SCRAPING ALL LOCATION POSTS (PROPER) ===\n")
...
for batch_num in range(total_batches):
    ...
    print(f"Scraping {len(location_ids)} locations with {POSTS_PER_LOCATION} posts each...")
    ...
    payload = {
        ...
        "maxItems": POSTS_PER_LOCATION * len(location_ids)  # CRITICAL: Total posts for batch
    }
    ...
    print(f"📦 Got {len(results)} posts from batch")
    ...
    print(f"💾 Saved to: crowd_posts_batch_{batch_num + 1}.json")
    print(f"🎯 Total posts so far: {all_posts_count}")
...
print(f"📊 Expected: ~{len(venues) * POSTS_PER_LOCATION} posts")
...
print(f"\n📸 DATABASE:")
print(f"   Venues with photos: {stats[0]}/{len(venues)}")
