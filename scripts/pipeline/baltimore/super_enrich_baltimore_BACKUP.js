...
const INPUT_FILE = "google_filtered_baltimore.json";
const OUTPUT_FILE = "super_enriched_baltimore.json";
...
async function analyzeWithOpenAI(venue, googleReviews, yelpData) {
  const prompt = `Analyze ${venue.venueName} in Baltimore for Lumina's nightlife concierge:
...
async function run() {
  ...
  console.log(`🧠 ENRICHING ${venues.length} Baltimore venues with OpenAI`);
  ...
  process.exit(0); // STOP HERE - NO LOOPING!
}
