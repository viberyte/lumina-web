...
const INPUT_FILE = "super_enriched_baltimore.json";
const OUTPUT_FILE = "tiktok_enriched_baltimore.json";
...
async function classify(venue, tiktokData) {
  ...
  const prompt = `Analyze TikTok videos for ${venue.venueName} in Baltimore.

Videos (${tiktokData.length} found):
${JSON.stringify(tiktokData.slice(0, 5).map(v => ({
  text: v.text,
...
async function main() {
  ...
  console.log(`🎥 TIKTOK ENRICHING Baltimore:`);
  ...
  for (let i = 0; i < remaining.length; i++) {
    ...
    const query = `${venue.venueName} Baltimore`;
    ...
    if (tiktokData.length > 0) {
      ...
      enriched.push({
        ...venue,
        tiktok_data: tiktokData.slice(0, 5).map(v => ({
          id: v.id,
          text: v.text,
          videoUrl: v.videoUrl,
      ...
      console.log(`   ⚠️  No TikToks found`);
      enriched.push({
        ...venue,
        tiktok_data: [],
