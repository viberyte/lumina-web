...
const INPUT_FILE = "google_filtered_philly.json";
const OUTPUT_FILE = "super_enriched_philly.json";
...
async function analyzeWithOpenAI(venue, googleReviews, yelpData) {
  const prompt = `Analyze ${venue.venueName} in Philadelphia for Lumina's nightlife concierge:
...
async function run() {
  ...
  console.log(`🧠 SUPER ENRICHING ${venues.length} Philly venues with OpenAI`);
