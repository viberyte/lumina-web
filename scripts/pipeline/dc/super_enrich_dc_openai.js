...
async function analyzeWithOpenAI(venue, googleReviews, yelpData) {
  const prompt = `Analyze ${venue.venueName} in Washington DC for Lumina's nightlife concierge:
...
async function run() {
  ...
  const existingEnriched = fs.existsSync(OUTPUT_FILE) 
    ? JSON.parse(fs.readFileSync(OUTPUT_FILE, "utf8"))
    : [];
  
  const enrichedNames = new Set(existingEnriched.map(v => v.venueName));
  const remainingVenues = allVenues.filter(v => !enrichedNames.has(v.venueName));
  
  console.log(`🧠 CONTINUING DC ENRICHMENT with OpenAI`);
  console.log(`✅ Already enriched: ${existingEnriched.length}`);
  console.log(`🔄 Remaining: ${remainingVenues.length}`);
  
  const enriched = [...existingEnriched];
  
  for (let i = 0; i < remainingVenues.length; i++) {
    console.log(`\n[${existingEnriched.length + i + 1}/${allVenues.length}]`);
    
    const enrichedVenue = await enrichVenue(remainingVenues[i]);
  ...
  console.log(`🍽️  Menus needed: ${menuScrapingList.length}`);
  console.log(`🎉 Events needed: ${eventScrapingList.length}`);
}
