...
const OUTPUT_FILE = "super_enriched_dc.json";
...
async function getYelpData(venueName, address) {
  try {
    ...
    if (!business) return { reviews: [], menuUrl: null, photos: [] };
    ...
    return { reviews: [], menuUrl: null, photos: [] };
  }
}

async function analyzeWithClaude(venue, googleReviews, yelpData) {
  const prompt = `Analyze ${venue.venueName} for Lumina's nightlife concierge app:

VENUE INFO:
- Website: ${venue.website || 'none'}
- Price Level: ${venue.priceLevel ? '$'.repeat(venue.priceLevel) : 'unknown'}
...
Provide comprehensive intelligence in EXACT JSON format:
{
  ...
  "average_entree_price": 25 or null (estimate from reviews),
  "budget_friendly_items": ["burger $12", "pasta $18"] or [],
  
  "needs_menu_scraping": true/false,
  "needs_event_scraping": true/false,
  
  "customer_insights": "2-3 sentence summary based on reviews"
}
...
async function enrichVenue(venue) {
  ...
  console.log(`  ✓ Got ${googleReviews.length} Google + ${yelpData.reviews.length} Yelp reviews`);
  
  if (googleReviews.length === 0 && yelpData.reviews.length === 0) {
    console.log("  ⚠️ No reviews found, using basic data");
    return {
      ...venue,
      needs_menu_scraping: venue.category === 'dining',
      needs_event_scraping: venue.category === 'nightlife' || venue.category === 'lounge',
      yelpUrl: yelpData.yelpUrl
    };
  }
  
  console.log("  → Analyzing with Claude...");
  const analysis = await analyzeWithClaude(venue, googleReviews, yelpData);
...
async function run() {
  ...
  console.log(`🧠 SUPER ENRICHING ${venues.length} DC venues`);
  console.log(`📊 Extracting: Vibes, Menus, Events, Happy Hours, Prices`);
  ...
  for (let i = 0; i < venues.length; i++) {
    ...
    // Rate limit: 2 seconds between venues
    await new Promise(resolve => setTimeout(resolve, 2000));
  }
  
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(enriched, null, 2));
  
  // Create scraping TODO lists
  ...
  console.log(`\n✅ COMPLETE!`);
  console.log(`📄 Enriched: ${enriched.length} venues`);
  console.log(`🍽️  Menu scraping needed: ${menuScrapingList.length} venues`);
  console.log(`🎉 Event scraping needed: ${eventScrapingList.length} venues`);
}
