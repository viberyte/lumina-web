...
async function runScrape() {
  console.log('LUMINA GOOGLE SCRAPER - RESUME (Queens → Philly)\n');

  // Load already-seen place IDs from completed scrapes
  const seenIds = new Set();
  const existingFiles = ['scrape_manhattan.json', 'scrape_brooklyn.json'];
  ...
  console.log(`Deduping against ${seenIds.size} already-scraped venues\n`);
  ...
  for (const city of CITIES) {
    ...
    // Update master results (merge all)
    const allFiles = ['scrape_manhattan.json','scrape_brooklyn.json'];
    let master = [];
