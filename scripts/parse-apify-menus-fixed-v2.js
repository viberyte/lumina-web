...
function isValidUrl(urlString) {
  if (!urlString || typeof urlString !== 'string' || urlString.trim().length < 5) {
    return false;
  }
  try {
...
async function main() {
  console.log('🔥 APIFY EXTRACTOR V2 - WITH STOP CONDITION\n');
  ...
  // ONLY get venues with VALID URLs
  ...
  console.log(`Processing ${venues.length} venues with valid websites\n`);
  ...
  for (const venue of venues) {
    ...
    const existingMenus = db.prepare(`SELECT COUNT(*) as count FROM menu_items WHERE venue_id = ? AND source = 'apify'`).get(venue.id);
    if (existingMenus.count > 0) {
      console.log(`\n[${stats.processed}/${venues.length}] ${venue.name} - ⏭️  ALREADY PROCESSED (${existingMenus.count} items)`);
    ...
    try {
      ...
      console.log('   ⚠️  URL parse error - skipping');
      stats.skipped++;
      continue;
    }
    ...
    // MENU ITEMS - USE INSERT OR IGNORE
    if (data.menu && data.menu.length > 0) {
      console.log(`   Menu: ${data.menu.length} items`);
      stats.menusFound++;
      stats.totalMenuItems += data.menu.length;
      
      const insertItem = db.prepare(`INSERT OR IGNORE INTO menu_items (venue_id, category, item_name, description, price, is_signature, source) VALUES (?, ?, ?, ?, ?, ?, 'apify')`);
    ...
    if (data.events && data.events.length > 0) {
      ...
      const insertEvent = db.prepare(`INSERT OR IGNORE INTO venue_events (venue_id, title, event_date, event_time, description, recurring) VALUES (?, ?, ?, ?, ?, ?)`);
    ...
    if (stats.processed % 10 === 0) {
      console.log(`\n📊 PROGRESS: ${stats.processed}/${venues.length} | Already Done: ${stats.alreadyProcessed} | New Menus: ${stats.menusFound} (${stats.totalMenuItems} items) | Events: ${stats.totalEvents}`);
    }
  }
  ...
  console.log(`\n\n✅ EXTRACTION COMPLETE!`);
  console.log(`Total Processed: ${stats.processed} venues`);
  console.log(`Already Had Data: ${stats.alreadyProcessed}`);
  console.log(`Skipped (invalid): ${stats.skipped}`);
  console.log(`New Menus: ${stats.menusFound} (${stats.totalMenuItems} items)`);
  console.log(`Events: ${stats.eventsFound} venues (${stats.totalEvents} total)`);
  console.log(`Happy Hours: ${stats.happyHours}`);
  console.log(`Bios Enhanced: ${stats.biosUpdated}`);
  console.log(`\n🛑 STOPPED AT ${venues.length} - WILL NOT LOOP!`);
}
