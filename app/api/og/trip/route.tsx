import { ImageResponse } from 'next/og';
import { NextRequest } from 'next/server';
import { supabaseServer } from '@/lib/supabase-server';

export const runtime = 'edge';

async function getTrip(id: string) {
  const { data, error } = await supabaseServer
    .from('lumina_trips')
    .select('*')
    .eq('id', id)
    .eq('is_public', true)
    .single();

  if (error || !data) return null;
  return data;
}

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const tripId = searchParams.get('id');

    if (!tripId) {
      return new Response('Missing trip ID', { status: 400 });
    }

    const trip = await getTrip(tripId);

    if (!trip) {
      return new Response('Trip not found', { status: 404 });
    }

    const venues = trip.venues || [];
    const venueCount = venues.length;
    const venueText = venueCount === 1 ? '1 place' : `${venueCount} places`;
    const heroImage = venues[0]?.venueImage;
    
    // Truncate long trip names
    const title = trip.name.length > 40 ? trip.name.slice(0, 37) + '…' : trip.name;

    return new ImageResponse(
      (
        <div
          style={{
            height: '100%',
            width: '100%',
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
            justifyContent: 'center',
            background: 'linear-gradient(135deg, #09090b 0%, #1a1025 50%, #09090b 100%)',
            position: 'relative',
          }}
        >
          {/* Background Image */}
          {heroImage && (
            <img
              src={heroImage}
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                objectFit: 'cover',
                opacity: 0.3,
              }}
            />
          )}

          {/* Gradient Overlay */}
          <div
            style={{
              position: 'absolute',
              width: '100%',
              height: '100%',
              background: 'linear-gradient(135deg, rgba(168,85,247,0.2), rgba(236,72,153,0.2), rgba(0,0,0,0.8))',
            }}
          />

          {/* Content */}
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              justifyContent: 'center',
              position: 'relative',
              zIndex: 1,
            }}
          >
            {/* Trip Name */}
            <div
              style={{
                fontSize: 72,
                fontWeight: 'bold',
                color: '#fff',
                textAlign: 'center',
                marginBottom: 30,
                textShadow: '0 4px 20px rgba(0,0,0,0.8)',
                maxWidth: '90%',
              }}
            >
              {title}
            </div>

            {/* Venue Count */}
            <div
              style={{
                fontSize: 36,
                color: '#d8b4fe',
                textAlign: 'center',
                marginBottom: 40,
                textShadow: '0 2px 10px rgba(0,0,0,0.8)',
              }}
            >
              📍 {venueText}
            </div>

            {/* Venue Preview */}
            {venues.length > 0 && (
              <div
                style={{
                  display: 'flex',
                  gap: 20,
                  marginBottom: 40,
                  flexWrap: 'wrap',
                  justifyContent: 'center',
                  maxWidth: '90%',
                }}
              >
                {venues.slice(0, 3).map((venue: any, i: number) => (
                  <div
                    key={i}
                    style={{
                      padding: '12px 24px',
                      background: 'rgba(168, 85, 247, 0.2)',
                      border: '2px solid rgba(168, 85, 247, 0.5)',
                      borderRadius: 25,
                      color: '#e9d5ff',
                      fontSize: 20,
                    }}
                  >
                    {venue.venueName.length > 25 ? venue.venueName.slice(0, 22) + '…' : venue.venueName}
                  </div>
                ))}
                {venues.length > 3 && (
                  <div
                    style={{
                      padding: '12px 24px',
                      background: 'rgba(168, 85, 247, 0.2)',
                      border: '2px solid rgba(168, 85, 247, 0.5)',
                      borderRadius: 25,
                      color: '#e9d5ff',
                      fontSize: 20,
                    }}
                  >
                    +{venues.length - 3} more
                  </div>
                )}
              </div>
            )}

            {/* Viberyte Branding */}
            <div
              style={{
                fontSize: 28,
                color: '#a855f7',
                fontWeight: 'bold',
                letterSpacing: 2,
              }}
            >
              LUMINA
            </div>
          </div>
        </div>
      ),
      {
        width: 1200,
        height: 630,
      }
    );
  } catch (error: any) {
    console.error('OG Image generation error:', error);
    return new Response(`Failed to generate image: ${error.message}`, { status: 500 });
  }
}
